...
class Bet365 extends FeedsAppModel
{
    ...
    {
        if (!isset($ImportedEvent['Bet'])) {
            return array();
        }

        if (!is_array($ImportedEvent['Bet'])) {
            return array();
        }

        if (empty($ImportedEvent['Bet'])) {
            return array();
        }

        foreach ($ImportedEvent['Bet'] AS $Bet) {
            if (!isset($Bet['Bet']['import_id'])) {
                continue;
            }
...
//var_dump($Bookmaker); exit;
    ...
    public function inplay()
    {
        ...
        for ($i = 0; $i < count($results); $i++) {
            $res = $results[$i];
            if ($res->type == "EV") { //event
                $event = new stdClass();
                $i++;
                $ma = $results[$i];
                if ($ma->type == "MA") {
                    $event->ma = array('id' => $ma->ID, 'name' => $ma->NA);
                    $i++;
                    $Odds = array();
                    while ($results[$i]->type == "PA") {
                        $Odds[] = array('id' => $results[$i]->ID, 'odds' => $this->CalcOdd($results[$i]->OD), 'name' => $results[$i]->NA);
                        $i++;
                    }
                    $event->odds = $Odds;
                    $events[$res->FI] = $event;
    ...
    public function CalcOdd($str)
    {
        $pos=strpos($str,"/");
        $odd1=substr($str,0,$pos);
        $odd2=substr($str,$pos+1);
//        echo $str.'- ';
        if ($odd2=="0")
            return 1;
        try {
