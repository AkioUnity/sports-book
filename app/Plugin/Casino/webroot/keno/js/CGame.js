function CGame(oData){
    ...
    this.play = function(){
		var data = {
			"numbers": _aListSelected,
			"bet": _iCurBet
		};
		request = $.ajax({
			type: "post",
			url: "/eng/casino/games/kenoPlay",
			dataType: "json",
		...
		request.done(function (response, textStatus, jqXHR){
			...
			self._clearAllExtracted();

			if(_iTotalNum < 2){
				return;
			}
			
			self.smartBlockGUI(false);
			...
			_bWin = response.win;
			_aCombination = response.combination;
			_iHitsNumber = response.hitsNum;
			
			self._animExtraction();
