...
if ($plugins = CakePlugin::loaded()) {
    ...
    foreach ($prefixes as $prefix) {
        ...
        Router::connect("/:language/{$prefix}/:plugin", $indexParams, $shortParams);
        Router::connect("/:language/{$prefix}/:plugin/:controller", $indexParams, $match);
        Router::connect("/:language/{$prefix}/:plugin/:controller/:action/*", $params, $match);
    }
    Router::connect('/:language/:plugin', array('action' => 'index'), $shortParams);
    Router::connect('/:language/:plugin/:controller', array('action' => 'index'), $match);
    Router::connect('/:language/:plugin/:controller/:action/*', array(), $match);
}

foreach ($prefixes as $prefix) {
    $params = array('prefix' => $prefix, $prefix => true);
    $indexParams = $params + array('action' => 'index');
    Router::connect("/:language/{$prefix}/:controller", $indexParams);
    Router::connect("/:language/{$prefix}/:controller/:action/*", $params);
}
