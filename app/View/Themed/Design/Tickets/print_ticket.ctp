<div id="main" class="ticket-print">
    ...
    <table>
        ...
        <tr>
            <td>
                <?php if($ticket["User"]["isStaff"] == true): ?>
                    <?php echo __('Operator'); ?>
                <?php else: ?>
                    <?php echo __('Placer'); ?>
                <?php endif; ?>
            </td>
            <td class="align-right"><?php printf('%s', $ticket["User"]["username"]); ?></td>
        </tr>
    </table>
        ...
        <?php if(intval(Configure::read('Settings.service_fee')) > 0): ?>
            <tr>
                <td class="align-right">
                    <?php echo __('Service fee ( %s %s ): %s %s', Configure::read('Settings.service_fee'), '%', $ticket['Ticket']['amount'] - $this->Beth->calculatePercentageGiven($ticket['Ticket']['amount'], Configure::read('Settings.service_fee'), Configure::read('Settings.service_fee'), intval(Configure::read('Settings.balance_decimal_places'))), Configure::read('Settings.currency')); ?>
                </td>
            </tr>
        <?php endif; ?>
        <tr>
            <td class="align-right"><?php echo __('Total amount'); ?>: <?php echo $ticket['Ticket']['amount']; ?> <?php echo Configure::read('Settings.currency'); ?></td>
        </tr>
        <tr>
            <td class="align-right">
                <?php echo __('Return:'); ?>
                <?php echo $this->Beth->convertCurrency($ticket['Ticket']['return']) . ' ' .  Configure::read('Settings.currency'); ?>
            </td>
        </tr>
    </table>
    <br />
    <div style="text-align: left;"><?php echo CakeSession::read('Auth.User.operator_signature'); ?></div>
</div>