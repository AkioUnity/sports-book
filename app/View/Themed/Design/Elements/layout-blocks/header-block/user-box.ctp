<?php if (!$this->Session->check('Auth.User') AND Configure::read('Settings.login') == 1): ?>
    <div id="login-form">
        ...
        <div class="form-inputs2">
            <input class="login-input icon-pass" name="data[User][password]" placeholder="Password"
                   type="password"/><br/>
...
<?php else: ?>
    <div id="loged">
        <div class="usr-pl">
            <?php echo $this->MyHtml->link(CakeSession::read('Auth.User.username'), array('language' => Configure::read('Config.language'), 'plugin' => false, 'controller' => 'tickets', 'action' => 'index')); ?>
        </div>
        <div class="usr-mo">
<!--            <a href="/eng/users/refresh_casino">-->
                <?php echo sprintf("%s %s", Configure::read('Settings.currency'), number_format((float)CakeSession::read('Auth.User.balance'), intval(Configure::read('Settings.balance_decimal_places')), '.', '')) ?>
<!--            </a>-->
        </div>
