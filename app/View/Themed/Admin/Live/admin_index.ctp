                            ...
                            <div class="span10">
                                <div class="widget">
                                    <div class="widget-title">
                                        <h4><i class="icon-user"></i> <?= $sports[$League["sport_id"]]; ?>
                                            / <?= $League["name"]; ?> / <?= $Event["name"]; ?></h4>
                                        <span class="tools">
                                    ...
                                    <div class="widget-body">
                                        <div>
                                            <label for="suspend">
                                                ...
                                                <input name="suspend_event[]" data-id="<?= $Event["id"]; ?>"
                                                       type="checkbox"
                                                       <?php if ($Event["active"] == Event::EVENT_INACTIVE_STATE): ?>checked="checked"<?php endif; ?>
                                                       type="checkbox">
                                            </label>
                                            <div>
                                                <div style="float: left;"><?php echo __("ID: %s", $Event["import_id"]); ?></div>
                                                <div style="float: right;"><?php echo __("Current time: %s min", round($Event["duration"] / 60)); ?></div>
                                                <div style="clear: both;"></div>
                                            </div>
                                            <div>
                                                <div style="float: left;"><?php echo __("League: %s", $League["name"]); ?></div>
                                                <div style="float: right;"><?php echo __("Current score: %s", $Event["result"]); ?></div>
                                                <div style="clear: both;"></div>
                                            </div>
                                            <div><?php echo __("Event: %s", $Event["name"]); ?></div>
                                            <div>
                                                        ...
                                                        foreach ($bet['BetPart'] as $betPart):
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td class="<?php echo $class; ?>">
                                                                    <input name="update_odd[]"
                                                                           data-id="<?= $betPart["id"]; ?>" type="text"
                                                                           value="<?= $betPart['odd']; ?>">
                                                                </td>
                                                                <td class="<?php echo $class; ?>">
                                                                    <input name="suspend_odd[]"
                                                                           data-id="<?= $betPart["id"]; ?>"
                                                                           type="checkbox"
                                                                           <?php if ($betPart["suspended"] == 1): ?>checked="checked"<?php endif;
                                                                    ?>>
                                                                </td>
                                                            </tr>
                                                        <?php endforeach; ?>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('input[name="suspend_event[]"]').change(function () {
            $.post(update, {
                "type": "suspend_event",
                Event: {id: $(this).attr('data-id'), value: $(this).is(':checked')}
            });
        });
